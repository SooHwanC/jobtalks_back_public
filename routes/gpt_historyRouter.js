...
router.post('/job', async (req, res, next) => {
    try {
        ...
        const createdRecord = await Gpt_history.create({ member_id, user_text, gpt_answer, company_name, answer_type });
...
router.post('/getAnswer', async (req, res, next) => {
    try {
        const { id } = req.body;
        const history = await Gpt_history.find({ _id: id });
...
router.post('/getAllAnswer', async (req, res, next) => {
    try {
        ...
        const history = await Gpt_history.find({ member_id: member_id });
